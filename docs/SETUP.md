...
| **AlphaFold structures** | Variants with a protein change trigger a live call to the AlphaFold Protein Structure Database (`https://alphafold.ebi.ac.uk/api/prediction/{accession}`), which serves a pre-computed structure per UniProt accession. No key, no GPU, no account. **These structures are wild-type.** The database holds one fold per accession, of the canonical sequence, so nothing here is mutation-specific. Producing a mutant structure would need AlphaFold 3 inference, which this repo does not run — AlphaFold Server has no submission API and its output terms forbid feeding its results to a docking tool, so the only route is self-hosted AF3 (non-commercial, approved weights, GPU). See `ai/services/alphafold.py`. |
