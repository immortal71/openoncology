...
| R15 | A tied set of candidates is presented to a clinician as a ranked list, so rank order carries information it does not have | Absent evidence sources have their weight redistributed, so equal-evidence candidates score identically and the tie breaks alphabetically (F16) | 4 | **OBSERVED** | The report now names the tie: `_annotate_ties` marks equally-supported candidates and the report prints "the order among them is not a preference". Ordering is deterministic and pinned by `test_ranking_ties.py` | Reduced, open | What should break a clinical tie: line of therapy, toxicity, route, cost. Which tied drugs make a three-slot cut is still decided by spelling. Belongs with open action 6 |
| R16 | A stale or wrong evidence-table entry is now the only candidate shown, with no broader pool underneath it to correct it | `candidate_pool_policy` defaulted to `evidence_first` on 2026-08-19: when the table has an answer it replaces the repurposing pool rather than reordering it | 4 | ASSUMED | Empty, resistance-only, and failed-lookup tables all fall back to the full pool (`test_candidate_pool_policy.py`); evidence provenance is stamped on every result (0013) and a sustained stale table alarms | **Open** | An answer key covering emerging and off-label biomarkers. Every gene measured was an approved indication, exactly where the table should be right, so the measurement cannot see this failure mode |
