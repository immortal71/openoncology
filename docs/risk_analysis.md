...
| 2 | Measured against the pool production uses (NCI-MATCH, now `evidence_first`): **46.9% exact Top-3, 78.1% class**. An independent FDA-label answer key now exists (`build_fda_label_answer_key.py`, 50 pairs / 21 genes, from a source the engine never reads) and favours the `evidence_first` pool by **+15.1 points on Precision@3, 7 wins to 0, p=0.016**. `candidate_pool_policy` exists; **default unchanged pending a human decision**. Remaining: a key covering emerging or off-label biomarkers, where the table may be wrong rather than absent | H5 | Partly |
