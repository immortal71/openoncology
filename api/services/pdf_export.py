...
def _build_patient_html(s: dict) -> str:
  ...
  <table>
    ...
    <tbody>{drug_rows}</tbody>
        ...
        if drugs
        else '<div class="box-warn"><strong>No specific treatment options were identified</strong> '
  ...
  <div class="box-warn"><ul>{limitation_items}</ul></div>"""
        if limitations
        else ""
    )
    about_block = (
...
def _build_oncologist_html(r) -> str:
    ...
    # Hoisted out of the template below. A triple-quoted f-string nested inside
    # another f-string is legal on Python 3.12 and a SyntaxError on 3.11, which
    # is what api/Dockerfile runs, so this module could not be imported in
    # production at all. Nothing in the suite imported it, and the route imports
    # it lazily, so the app started normally and only the two download endpoints
    # raised. Keeping these as plain locals means the template interpolates
  ...
  <table>
    ...
    <tbody>{top3_rows}</tbody>
        ...
        if top3_rows
        else "<p>No ranked drug candidates identified.</p>"
    )
    genomic_block = (
        f"""<table>
    <thead>
    ...
    <tbody>{genomic_rows}</tbody>
        ...
        if genomic_rows
        else "<p>No genomic alteration data provided.</p>"
    )
    drug_table_block = (
        f"""<table>
    <thead>
  ...
  {exp_html}"""
    ...
    how_to_use_items = "".join(
        f"<li>{html.escape(str(item))}</li>" for item in how_to_use
    )
    how_to_use_block = (
        f"""<h2>Section 5 — How to Use This Report</h2>
  <ul>{how_to_use_items}</ul>"""
        if how_to_use
        else ""
    )
